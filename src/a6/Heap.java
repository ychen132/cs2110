...
public final class Heap<T> {
    ...
    void swap(int h, int k) {
        ...
        Pair swapping= new Pair(b[h].value, b[h].priority);
        map.put(b[h].value, k);
        map.put(b[k].value, h);
        b[h]= b[k];
        b[k]= swapping;
    ...
    void bubbleUp(int h) {
        ...
        int parentIndex= (h - 1) / 2;
        while (compareTo(h, parentIndex) == 1) {
            swap(h, parentIndex);
            h= parentIndex;
            parentIndex= (h - 1) / 2;
        }
    ...
    void bubbleDown(int h) {
        ...
        assert h >= 0 && h < size;
        int childIndex= h * 2 + 1;
        while (childIndex < size) {
            int max= findMax(childIndex, childIndex + 1);
            int compareResult= compareTo(max, h);
            if (compareResult == -1 || compareResult == 0) { return; }
            swap(max, h);
            h= max;
            childIndex= h * 2 + 1;
        }
    ...
    protected int findMax(int i, int j) {
        if (j >= size) {
            return i;
        } else {
            int compareResult= compareTo(i, j);
