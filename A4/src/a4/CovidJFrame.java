...
public class CovidJFrame extends JFrame {
	...
	/** Display the covid route from the previously selected person to the selected person. */
	private JLabel twoPersonCovidRouteLabel= new JLabel();
	...
	/** The covid route from the previously selected person to the currently selected person. */
	private List<Person> covidRoutePrevToSelected;
	...
	private void createAndAddCircleMouseListener() {
		graphPanel.addCircleMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					covidRoutePrevToSelected= root.node(previouslySelectedPerson)
						.covidRouteTo(selectedPerson);
...
//		infoPanel.setLayout(new BoxLayout(infoPanel, BoxLayout.Y_AXIS));
		...
		infoPanel.add(twoPersonCovidRouteLabel);
	...
	private void recLayoutNodes(CovidTree dt, float xMin, float xMax, float yPercent,
		...
		GraphPanel<Person, PersonConnection>.Circle c= graphPanel.getCircle(dt.person());
	...
	private void colorNodes(CovidTree dt) {
		GraphPanel<Person, PersonConnection>.Circle c= graphPanel.getCircle(dt.person());
		c.setColor(dt.person().isDead() ? DEAD_COLOR : IMMUNE_COLOR);
		for (CovidTree child : dt.copyOfChildren()) {
			colorNodes(child);
		}
	}
	...
	private void populateLabels() {
		...
		if (pp != null) {
			populateLabel(() -> {
				List<Person> route= root.node(pp).covidRouteTo(p);
				if (route == null) return null;
				else return Arrays.deepToString(route.stream().map(a -> a.name()).toArray());
			}, twoPersonCovidRouteLabel, "Covid Route");
			populateLabel(() -> {
