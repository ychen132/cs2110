...
public class A2 {
    ...
    public static boolean areMidsDiff(String s) {
        ...
        if (s.length() % 2 == 1 || s == null || s.length() == 0) { return false; }
    ...
    public static int nCat(String s, String x) {
        ...
        int x_len= x.length();
        if (s_len == 0) { return 0; }
        if (s_len != 0 && x_len == 0) { return -1; }
        if (s_len % x_len != 0) { return -1; }
        int result= 0;
        for (int i= 0; i < s_len; i+= x_len) {
